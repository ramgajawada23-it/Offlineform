...
public class Candidate {
    ...
    private Integer joiningDays; // Notice Period
    ...
    // ===== CTC Review Fields (filled by HR after submission) =====
    private String hrEmail; // HR who sent the form (stored at send time)
    private Double offeredCtc; // Offered CTC (Annual)
    private Double ctcBasic;
    private Double ctcHra;
    private Double ctcOther;
    @Column(length = 2000)
    private String ctcRemarks; // HR notes / remarks
    private Boolean ctcReviewDone; // Flag: true = HR has completed CTC review
    ...
    public void setLanguages(List<LanguageKnown> languages) {
        ...
        if (languages != null)
            languages.forEach(l -> l.setCandidate(this));
    }
    ...
    public void setRef(List<Reference> ref) {
        this.ref = ref;
        if (ref != null)
            ref.forEach(r -> r.setCandidate(this));
    }
